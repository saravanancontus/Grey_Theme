 ...
 */
class Mage_Adminhtml_Block_Sales_Order_Grid_Renderer_Giftorder extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Action
{
    /**
     * Renders column
     *
    ...
    public function render(Varien_Object $row)
    {
        ...
        $giftMessage = $read->fetchALL("Select gift_message_id,recipient   from " . $tprefix . "gift_message  where order_id =" .$row->getId(). "");

        if($giftMessage[0][gift_message_id] != '')
        {
            $resultGiftMessage = ucfirst($giftMessage[0][recipient]);
